...
void print_char(char character, int col, int row, char attribute_byte)
{
    ...
    if (col >= 0 && row >= 0)
    {
        offset = (int)get_screen_offset(col, row);
    } else
    {
    ...
    offset += 2;
    offset = (int)handle_scrolling(offset);
    set_cursor(offset);
}
...
int get_screen_offset(int cols, int rows)
{
    ...
    calculated_offset = 2 * ((rows - 1) * 80 + cols);
